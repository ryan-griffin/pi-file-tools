...
- **`rename`** moves a file, symlink, or directory to the exact destination. Existing destinations require `overwrite: true`; directory moves into their own descendants are rejected. Overwriting the filesystem root, home directory, active working directory, or any real directory containing them is refused.
...
- **`copy`** copies a regular file, symlink, or directory to the exact destination while preserving symlinks and source permission bits. Special filesystem entries are rejected. Directories require `recursive: true`, and existing destinations require `overwrite: true` (refused for the same protected destinations as `rename`). Overwrites are staged in a temporary sibling and swapped only after the copy succeeds, preserving the old destination if staging fails. Abort signals are checked before mutation and between recursive entries.
